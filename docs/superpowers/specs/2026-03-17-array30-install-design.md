...
2. 解析 PKGBUILD 的 `source=()` 欄位，拿到上游 tarball URL
3. 下載解壓上游原始碼
4. 在 `/tmp/array30-build-XXXX/` 暫存目錄中執行：
   ```bash
   cmake -B build \
     -DCMAKE_INSTALL_PREFIX=/usr \
     -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=/usr/lib/x86_64-linux-gnu/fcitx5 \
     -DCMAKE_BUILD_TYPE=Release
   cmake --build build
   ```
5. 手動安裝產出物到系統目錄（`sudo cp`），不用 `make install` 以保持精確控制
6. 建立 `libarray.so → array.so` symlink（Ubuntu fcitx5 loader 需要 `lib` 前綴）
7. 清理 build 暫存目錄
...
| `libassociation.so` | `/usr/lib/x86_64-linux-gnu/fcitx5/` |
...
- **內容：**
  ...
  - `metadata.txt` — 記錄備份時間、fcitx5 版本、array.db 筆數
...
cd array30-install
sudo bash array30-install.sh install
```

**curl | bash 模式：**
```bash
curl -fsSL https://raw.githubusercontent.com/<user>/array30-install/main/array30-install.sh | sudo bash -s -- install
```

### 安全考量

- 腳本開頭檢查是否為 pipe 模式（`[ -t 0 ]`），若是 pipe 則跳過互動確認
